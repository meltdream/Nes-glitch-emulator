...
int nes6502_execute(int timeslice_cycles)
{
   ...
   GET_GLOBAL_REGS();
   if (ext_irq_line && remaining_cycles > 0){
      nes6502_irq();               /* level-triggered behaviour */
   }
...
void nes6502_reset(void)
{
   ...
   cpu.irq_was_requested = 0;                /* Clear pending IRQ edge */
...
void nes6502_irq(void)
{
    ...
    if (!cpu.jammed) {
        GET_GLOBAL_REGS();
        if (i_flag == 0) {
            /* already unmasked – normal immediate IRQ */
            IRQ_PROC();
            cpu.burn_cycles += INT_CYCLES;
        } else {
            cpu.int_pending = 1;
            cpu.irq_was_requested = 1;     /* remember edge */
        }
        STORE_LOCAL_REGS();
    }
}
...
/* ------------------------------------------------------------------ */
/*  Public helper: cancel any queued edge-triggered IRQ               */
/* ------------------------------------------------------------------ */
void nes6502_clear_pending_irq(void)
{
    cpu.int_pending = 0;      /* cpu is in this translation unit   */
}
